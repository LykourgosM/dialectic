...
def test_costs_aggregates_by_writer_and_reviewer(
    ...
    _write_run(
        runs_dir, run_id="20260101-000001-aaaaaa",
        writer_model="claude-opus-4-7", reviewer_model="gpt-5.4", cost_usd=0.10,
    )
    _write_run(
        runs_dir, run_id="20260101-000002-bbbbbb",
        writer_model="claude-opus-4-7", reviewer_model="gpt-5.4", cost_usd=0.25,
    )
    _write_run(
        runs_dir, run_id="20260101-000003-cccccc",
        writer_model="claude-opus-4-7", reviewer_model="gpt-5.4", cost_usd=0.05,
    )
    # 2 runs: claude-sonnet-4-6 (writer) + gpt-5 (reviewer)
    _write_run(
        runs_dir, run_id="20260101-000004-dddddd",
        writer_model="claude-sonnet-4-6", reviewer_model="gpt-5", cost_usd=0.03,
    )
    _write_run(
        runs_dir, run_id="20260101-000005-eeeeee",
        writer_model="claude-sonnet-4-6", reviewer_model="gpt-5", cost_usd=0.07,
    )
    # 1 cross-pairing: claude-opus-4-7 (writer) + gpt-5 (reviewer)
    _write_run(
        runs_dir, run_id="20260101-000006-ffffff",
        writer_model="claude-opus-4-7", reviewer_model="gpt-5", cost_usd=0.40,
    )
    ...
    sonnet_writer_runs, sonnet_writer_cost = _find_row(
        result.output, "claude-sonnet-4-6", "writer"
    )
    ...
    total_match = re.search(r"Total spent across (\d+) run\(s\):\s*\$([0-9]+\.[0-9]+)", result.output)
...
def test_costs_skips_unparseable_files(tmp_path: Path, wide_console: None) -> None:
    ...
    _write_run(
        runs_dir, run_id="20260101-000001-aaaaaa",
        writer_model="claude-opus-4-7", reviewer_model="gpt-5.4", cost_usd=0.12,
    )
...
def test_costs_single_run_counted_once_in_total(
    tmp_path: Path, wide_console: None
) -> None:
    ...
    _write_run(
        runs_dir, run_id="20260101-000001-aaaaaa",
        writer_model="claude-opus-4-7", reviewer_model="gpt-5.4", cost_usd=0.42,
    )
