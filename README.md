...
> A personal CLI tool I use during hackathons and competitive programming sessions — runs Claude as a writer and Codex as a reviewer on the same coding task, so I can get a second pair of eyes on AI-generated code before committing under time pressure.

I started building this because most of the public multi-CLI orchestrators I'd looked at (10+ surveyed below) use voting, debate, or consensus patterns — patterns that recent literature suggests have known failure modes for code generation. I wanted to actually understand the design space before building, not just pick the trendy thing. So I went deep on the research, settled on the design the evidence supports (single writer, cross-family reviewer with execution access, structured per-item disagreement, human as the final arbiter), and built the boring infrastructure (tests, CI, sandbox checks) carefully because I'd rather debug protocol bugs once than fight subprocess timeouts at 3am during a hackathon.

Real head-to-head evidence vs. solo Claude on the same prompts is in [Head-to-head](#head-to-head-with-vs-without-the-reviewer) below.
...
Each agent invocation runs in an ephemeral `.dialectic/wt/{writer,reviewer}-<id>/` worktree that's deleted after the run. The writer holds workspace-write permissions; the reviewer also has workspace-write so it can actually execute tests/lints. Your main working tree is untouched until you approve.

See [`docs/architecture.md`](./docs/architecture.md) for a fuller diagram and the exact safety checks at apply time.

## Comparison with existing tools

The multi-CLI / multi-model code-orchestration space is crowded. Honest read on where this tool sits relative to the field as of May 2026:
...
This tool is deliberately narrow where the field is wide. It scopes to 2 CLIs and concentrates on a **per-item protocol where the reviewer can rebut, and where any unresolved dispute is a first-class step rather than a flag**. None of the others have a structured per-item defend-then-rebut loop. The honest cost: it's younger, smaller, and has fewer integrations than every tool above except `claude-codex-collab`.

## Head-to-head: with vs without the reviewer

To check whether the reviewer actually catches things a single Claude run would miss, I ran the same three prompts through both pipelines — standalone `claude -p` and the full writer/reviewer dance — with the same writer model (Opus 4.7, max thinking), the same git base, and the same prompts.

| Prompt | Solo Claude | With reviewer | Items reviewer raised |
|---|---|---|---|
...
The honest caveat: round 2 then raised 2 *new* items that were introduced by the round-1 fixes, and the run ended in `AWAITING_ARBITRATION`. So on design-heavy prompts the workflow is *"writer + reviewer iterating, user adjudicates residuals"* — not a clean one-and-done.

Net: the reviewer earned its premium when the prompt had architectural ambiguity. On well-specified contained changes, solo Claude is equivalent for less time and money. The "[When NOT to use](#when-not-to-use-this)" section below reflects this.

## When NOT to use this

- **Well-specified, single-file changes.** On contained CLI extensions like `dialectic stats` and `dialectic show --last`, the [head-to-head](#head-to-head-with-vs-without-the-reviewer) showed the reviewer raised zero items and cost 2-3× what solo Claude did. Skip this; you'll get equivalent code from a plain `claude -p`.
- **Short prompts where speed matters more than scrutiny.** A `claude -p "rename foo to bar"` is 30 seconds. This adds at least the reviewer's round trip (minimum 2–3 minutes, $0.20+). Not worth it for trivial edits.
- **Throughput across many tasks.** If you have 50 independent issues to fix in parallel, you want `ComposioHQ/agent-orchestrator` (or similar) and its auto-CI-fix loop. This processes one prompt at a time on purpose.
- **Production CI/CD pipelines without a human in the loop.** Disputes get surfaced to you. If "you" is a cron job, the unresolved disputes have nowhere to go. (`--auto-approve` exists but disables the human-arbitration safety; use sparingly.)
- **Closed-form QA / single-token outputs.** Plain self-consistency ([Wang et al. 2022](https://arxiv.org/abs/2203.11171)) outperforms fancier patterns on these, and a two-agent pipeline has nothing to add.
- **If you only have one CLI installed.** It needs both `claude` and `codex` on PATH; cross-family review is the whole point.
...
You'll need `claude` and `codex` on PATH and both authenticated. The CLIs use whatever subscription/auth you already have — no separate API keys needed.
...
Defaults are the highest-effort settings on both sides because that's what the cross-family review pattern is designed to exploit; expect $1–$3 per run and 5–15 minutes. Drop to `medium` effort for ~10× cheaper runs at the cost of catching fewer real issues.
...
This is a personal tool I use during hackathons and on the occasional competitive-programming session, not a product. 121 tests, CI green, real dogfood runs validated. Protocol works end-to-end including writer/reviewer/respond/rebut/arbitrate, with `base_sha`-anchored apply-time safety, audit logs, and per-run schema versioning.

A few things that would be nice but I haven't built because I haven't actually needed them yet:

- `ACCEPT_REVIEWER` arbitration choice (currently `NotImplementedError`). Would need a design for synthesizing patches from the reviewer's free-text `suggested_fix`. So far runs have either approved cleanly or escalated only items I could handle with `ACCEPT_WRITER` / `SKIP` + a manual paste.
- Multi-reviewer panel ([Verga et al. 2024 PoLL](https://arxiv.org/abs/2404.18796)). The protocol has `reviewer_id` ready, so it's non-breaking to add. Would require a third CLI installed (Gemini or similar) and critique-merging logic.
...
Papers and write-ups that informed the design. Full annotated bibliography in [`docs/architecture.md`](./docs/architecture.md).

<details>
<summary>Patterns adopted</summary>
...
<details>
<summary>Patterns rejected</summary>
